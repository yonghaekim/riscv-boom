 ...
 */
object MCUDecode extends DecodeConstants
{
           //                                                                  frs3_en                        wakeup_delay
           //     is val inst?                                                 |  imm sel                     |    bypassable (aka, known/fixed latency)
           //     |  is fp inst?                                               |  |     uses_ldq              |    |  is_br
  ...
  val table: Array[(BitPat, List[BitPat])] = Array(//  |       |       |       |  |     |  |  |  |  |  |      |    |  |  |  |  |  |
  // PA primitives
  TAGD    -> List(Y, N, X, uopTAGD , IQT_INT, FU_MUL , RT_FIX, RT_FIX, RT_FIX, N, IS_X, N, N, N, N, N, M_X,   1.U, Y, N, N, N, N, CSR.N),
  AUTD    -> List(Y, N, X, uopAUTD , IQT_INT, FU_MUL , RT_FIX, RT_FIX, RT_FIX, N, IS_X, N, N, N, N, N, M_X,   1.U, Y, N, N, N, N, CSR.N),
  ...
  with freechips.rocketchip.rocket.constants.MemoryOpConstants
{
  ...
  decode_table ++= MCUDecode.table //yh+
...
//     cs.div && !csr.io.status.isa('m'-'a') || TODO check for illegal div instructions
  ...
  uop.cap_cmd := Mux(inst(6,0) =/= BitPat("b0101011"), 0.U,
                    Mux(inst(14,12) === BitPat("b000"), 1.U, // cstr
                    Mux(inst(14,12) === BitPat("b001"), 2.U, // cclr
                    0.U)))
