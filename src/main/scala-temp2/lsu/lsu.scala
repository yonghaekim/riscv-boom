    ...
    with HasBoomUOP
{
	...
	val addr								= UInt((vaddrBits+1).W) // addr[31:0]
  ...
  with rocket.HasL1HellaCacheParameters
{
  ...
  val scq_commit_head  = Reg(UInt(scqAddrSz.W)) // point to next store to commit
  val scq_execute_head = Reg(UInt(scqAddrSz.W)) // point to next store to execute
  ...
  //yh+begin
  val clear_stchk = WireInit(false.B) // store
  val clear_ldchk = WireInit(false.B) // load
  val clear_capst = WireInit(false.B) // cstr, cclr
  val live_tag_mask = RegInit(0.U(numScqEntries.W))
  var next_live_tag_mask = Mux(clear_capst, live_tag_mask & ~(1.U << ssq_head),
                                                  live_tag_mask)
  //yh+end
  ...
  for (i <- 0 until numSlqEntries) {
    when (clear_capst) {
      slq(i).bits.tag_dep_mask := slq(i).bits.tag_dep_mask & ~(1.U << scq_head)
    }
  }

  for (i <- 0 until numSsqEntries) {
    when (clear_capst) {
      ssq(i).bits.tag_dep_mask := ssq(i).bits.tag_dep_mask & ~(1.U << scq_head)
    }
  }

  for (i <- 0 until numScqEntries) {
    when (clear_capst) {
      scq(i).bits.tag_dep_mask := scq(i).bits.tag_dep_mask & ~(1.U << scq_head)
    }
  } 
  //yh+end
  ...
  for (w <- 0 until coreWidth)
  {
      ...
      .elsewhen (dis_sc_val)
    {
      ...
      //scq(sc_enq_idx).bits.uop.uses_ldq   := false.B // need to be cleared later to go through TLB
      //scq(sc_enq_idx).bits.uop.uses_stq   := false.B // need to be cleared later to go through TLB
      scq(sc_enq_idx).bits.uop.is_cap     := true.B
      scq(sc_enq_idx).bits.uop.mem_size   := 3.U
      //scq(sc_enq_idx).bits.uop.mem_cmd    := io.core.dis_uops(w).bits
      scq(sc_enq_idx).bits.data.valid     := false.B
      scq(sc_enq_idx).bits.committed      := false.B
      scq(sc_enq_idx).bits.succeeded      := false.B
      scq(sc_enq_idx).bits.state          := s_init
      scq(sc_enq_idx).bits.tagged         := false.B
			scq(sc_enq_idx).bits.failed			    := false.B
      scq(sc_enq_idx).bits.tag_dep_mask   := next_live_tag_mask
      scq(sc_enq_idx).bits.ccache_hit     := false.B
      scq(sc_enq_idx).bits.is_rob_head    := false.B

      printf("[%d] Dispatch scq(%d) tag_mask: %x stq(%d) rob(%d) cstr: %d cclr: %d\n",
							...
							io.core.dis_uops(w).bits.cap_cmd === CAP_CC)
    }
 ...
 // Prioritize emptying the store queue when it is almost full
  ...
  val slq_update_e = slq(slq_update_idx)
	...
	val can_update_ldchk = widthMap(w =>
													...
													!io.core.exception                  												&&
													(slq_update_e.bits.tag_dep_mask.asUInt === 0.U)							&&
  ...
  val ssq_update_e = ssq(ssq_update_idx)
	...
	val can_update_stchk = widthMap(w =>
													...
													!io.core.exception                  												&&
													(ssq_update_e.bits.tag_dep_mask.asUInt === 0.U) 						&&
													!IsKilledByBranch(io.core.brupdate, ssq_update_e.bits.uop)	&&
													(w == 0).B)

	// SCQ
  val scq_update_idx = RegNext(AgePriorityEncoder((0 until numScqEntries).map(i => {
    val e = scq(i).bits
		e.state === s_update
  }), scq_head))
  val scq_update_e = scq(scq_update_idx)

	val can_update_capst = widthMap(w =>
													scq_update_e.valid									        								&&
													(scq_update_e.bits.state === s_update)	    								&&
													!io.core.exception                  												&&
													(scq_update_e.bits.tag_dep_mask.asUInt === 0.U) 						&&
  ...
  val ccache_meta = Reg(Vec(256, UInt((tagWidth/2).W)))
	val ccache_data = Reg(Vec(256, UInt((vaddrBits*2).W)))
	...
	for (w <- 0 until memWidth) {
		...
		when (can_update_ldchk(w)) {
			...
			// Clear needCC
			clr_needCC_valid(w)	:= (!slq_update_e.bits.tagged || hit)
		...
		// Update SSQ
			...
			ssq_update_e.bits.way					:= 0.U //TODO check...
			ssq_update_e.bits.state				:= Mux(hit || !tagged, s_done, s_fire)
			ssq_update_e.bits.ccache_hit	:= hit
		...
		when (can_update_capst(w)) {
			...
			val hit = (high4 === meta)
			val cstr_head = shb_data(low4)
			val cclr_head = chb_data(low4)

			scq_update_e.bits.way			:= Mux(hit, Mux(is_cstr, cstr_head, cclr_head), 0.U) //TODO check...
			scq_update_e.bits.state		:= s_fire

			when (is_cstr) {
				printf("[%d] SHB Lookup! scq(%d) cstr_head: %d\n",
								io.core.tsc_reg, scq_update_idx, cstr_head)
			} .otherwise {
				printf("[%d] CHB Lookup! scq(%d) cclr_head: %d\n",
								io.core.tsc_reg, scq_update_idx, cclr_head)
			}
		}
	}
  ...
  val can_fire_capst = widthMap(w =>
												...
												!scq_fire_e.bits.failed               										&&
												!scq_fire_e.bits.succeeded            										&&
												!io.core.exception                  											&&
												!store_needs_order                      									&&
												!mem_xcpt_valid                         									&&
												(w == 0).B)
  ...
  val will_fire_cap_store = Wire(Vec(memWidth, Bool()))
	...
	val slq_fire_vaddr = widthMap(w => cmt_base + (slq_fire_e.bits.tag << tagOffset) +
                          (slq_fire_e.bits.way << wayOffset))

	val ssq_fire_uop = widthMap(w => ssq_fire_e.bits.uop) // TODO check brmask
	val ssq_fire_vaddr = widthMap(w => cmt_base + (ssq_fire_e.bits.tag << tagOffset) +
                          (ssq_fire_e.bits.way << wayOffset))
  ...
  val can_fire_ldchk_p1 = Reg(Vec(memWidth, Bool()))
	...
	for (w <- 0 until memWidth) {
		...
		cap_check_idx(w) := Mux(fire_ldchk_valid(w), slq_fire_idx(w), ssq_fire_idx(w))
	...
	val cap_vaddr = (cmt_base + (scq_fire_e.bits.tag << tagOffset) + (scq_fire_e.bits.way << wayOffset))
	val cap_store_vaddr = RegNext(widthMap(w => Mux(scq_fire_e.bits.state === s_fire,
													cap_vaddr, cap_vaddr | 8.U)))
	val cap_store_data = RegNext(widthMap(w => Mux(scq_fire_e.bits.state === s_fire,
												scq_fire_e.bits.addr, scq_fire_e.bits.data.bits)))
	val cap_store_idx = RegNext(widthMap(w => scq_head))

	for (w <- 0 until memWidth) {
		when (fire_ldchk_valid(w)) {
			printf("[%d] Can fire load check! slq(%d) vaddr: %x base: %x tag: %x way: %d\n",
							io.core.tsc_reg, slq_fire_idx(w), slq_fire_vaddr(w),
							cmt_base, slq_fire_e.bits.tag, slq_fire_e.bits.way)
		} .elsewhen (fire_stchk_valid(w)) {
			printf("[%d] Can fire store check! ssq(%d) vaddr: %x base: %x tag: %x way: %d\n",
							io.core.tsc_reg, ssq_fire_idx(w), ssq_fire_vaddr(w),
							cmt_base, ssq_fire_e.bits.tag, ssq_fire_e.bits.way)
		}

		when (can_fire_capst(w)) {
			printf("[%d] Can fire cap store! scq(%d) vaddr: %x base: %x tag: %x way: %d\n",
							io.core.tsc_reg, scq_head,
							Mux(scq_fire_e.bits.state === s_fire, cap_vaddr, cap_vaddr | 8.U),
							cmt_base, scq_fire_e.bits.tag, scq_fire_e.bits.way)
		}
	}

  //printf("[%d] next_live_tag_mask: %x\n", io.core.tsc_reg, next_live_tag_mask)

  val cstr_xcpt_valid = (scq_fire_e.valid	          							&&
													scq_fire_e.bits.data.valid							&&
													scq_fire_e.bits.uop.cap_cmd === CAP_CS	&&
													scq_fire_e.bits.failed              		&&
													(scq_fire_e.bits.state === s_done))
													//(scq_fire_e.bits.state === s_resp))
													//(scq_fire_e.bits.state === s_resp)	&&
													//!scq_fire_e.bits.committed)

  val cstr_xcpt_uop = scq_fire_e.bits.uop

	//val fired_cstrclr_clr_bsy = Reg(Vec(memWidth, Bool()))
	//for (w <- 0 until memWidth) {
	//	fired_cstrclr_clr_bsy(w) := false.B
	//}	
  //val cap_clr_bsy_valid = widthMap(w => scq_fire_e.valid												&&
	//																			scq_fire_e.bits.data.valid									&&
	//																			((scq_fire_e.bits.uop.cap_cmd === CAP_CS		&&
	//																				!scq_fire_e.bits.failed)									||
	//																				scq_fire_e.bits.uop.cap_cmd === CAP_CC)	&&
	//																			//!scq_fire_e.bits.failed							&&
	//																			(scq_fire_e.bits.state === s_resp)					&&
  ...
  for (w <- 0 until memWidth) {
    ...
    when (will_fire_cap_check(w)) {
      ...
      block_capst_mask(cap_store_idx(w)) := true.B
		...
		// TODO need to block cap store too?
		//yh+end
  ...
  val exe_kill   = widthMap(w =>
	...
	//yh+begin
	val cap_ldchk_incoming = widthMap(w => cap_exe_req(w).valid && cap_exe_req(w).bits.uop.ctrl.is_load)

	val cap_stchk_incoming = widthMap(w => cap_exe_req(w).valid && cap_exe_req(w).bits.uop.ctrl.is_sta
																					&& !cap_exe_req(w).bits.uop.cap_cmd(1))
  ...
  val ccache_write_data = Reg(Vec(memWidth, UInt((xLen*2).W)))
  for (w <- 0 until memWidth) {
    ...
    when (ccache_write_val(w)) {
      ...
      printf("[%d] Write C-Cache! tag: %x lbnd: %x ubnd: %x\n",
							io.core.tsc_reg, ccache_write_tag(w), ccache_write_data(w)(xLen-1,0), ccache_write_data(w)(xLen*2-1,xLen))
    }
  }
	//yh+end
  ...
  for (w <- 0 until memWidth) {
      ...
      .elsewhen (will_fire_cap_check(w))
    {
      ...
      when (dmem_req_fire(w)) {
	      ...
	      when (can_fire_ldchk_p1(w)) {
          printf("slq(%d) vaddr: %x paddr: %x\n", exe_tlb_uop(w).slq_idx, exe_tlb_vaddr(w), exe_tlb_paddr(w))
        } .otherwise {
          printf("ssq(%d) vaddr: %x paddr: %x\n", exe_tlb_uop(w).ssq_idx, exe_tlb_vaddr(w), exe_tlb_paddr(w))
        }
      }
      ...
      .elsewhen (will_fire_cap_store(w))
    {
      ...
      dmem_req(w).bits.data						:= (new freechips.rocketchip.rocket.StoreGen(
			...
			//TODO should use scq_execute_head or pipelined one?
      //scq_execute_head          := Mux(dmem_req_fire(w),
      //                               		WrapInc(scq_execute_head, numScqEntries),
      //                                  scq_execute_head)
      scq(scq_head).bits.succeeded := false.B
      ...
      when (dmem_req_fire(w)) {
				printf("[%d] Fire cap store! scq(%d) vaddr: %x paddr: %x cap_store_idx: %d scq_head: %d\n",
								io.core.tsc_reg, exe_tlb_uop(w).scq_idx, exe_tlb_vaddr(w),
								exe_tlb_paddr(w), cap_store_idx(w), scq_head)
      }

			assert(exe_tlb_uop(w).scq_idx === cap_store_idx(w))
		}
    //  .elsewhen (will_fire_capst(w))
    //{
    //  //dmem_req(w).valid      				:= !exe_tlb_miss(w) && !exe_tlb_uncacheable(w)
    //  dmem_req(w).valid         		:= true.B
    //  dmem_req(w).bits.addr     		:= scq_fire_e.bits.paddr
    //  //dmem_req(w).bits.addr     		:= exe_tlb_paddr(w)
    //  dmem_req(w).bits.data					:= (new freechips.rocketchip.rocket.StoreGen(
		//																		3.U, 0.U,
		//																		ssq_commit_data,
		//																		coreDataBytes)).data
    //  dmem_req(w).bits.uop      		:= scq_fire_e.bits.uop
    //  //dmem_req(w).bits.uop.uses_ldq := false.B
    //  dmem_req(w).bits.uop.uses_stq := false.B
    //  dmem_req(w).bits.uop.is_capld := false.B

    //  scq_execute_head          := Mux(dmem_req_fire(w),
    //                                 		WrapInc(scq_execute_head, numSsqEntries),
    //                                    scq_execute_head)

    //  ssq(scq_execute_head).bits.succeeded := false.B

    //  when (dmem_req_fire(w)) {
    //    printf("[%d] Fire commit ssq(%d) paddr: %x caddr: %x data: %x cstr: %d cclr: %d\n",
    //            io.core.tsc_reg, scq_fire_e.bits.uop.ssq_idx, scq_fire_e.bits.paddr, scq_fire_e.bits.caddr,
		//						ssq_commit_data, scq_fire_e.bits.uop.cap_cmd === CAP_CS, scq_fire_e.bits.uop.cap_cmd === CAP_CC)
    //  }
    //}
		//yh+end
    ...
    when (cap_ldchk_incoming(w)) {
			...
			slq(slq_idx).bits.uop			:= cap_exe_req(w).bits.uop
    ...
    when (cap_stchk_incoming(w)) {
			...
			ssq(ssq_idx).bits.uop			:= cap_exe_req(w).bits.uop
    ...
    when (cap_capsta_incoming(w)) {
			...
			scq(scq_idx).bits.uop			:= cap_exe_req(w).bits.uop
			scq(scq_idx).bits.addr		:= cap_exe_req(w).bits.addr
			scq(scq_idx).bits.dir			:= cap_exe_req(w).bits.dir
			scq(scq_idx).bits.count 	:= init_count
			scq(scq_idx).bits.state 	:= s_update

			printf("[%d] Exe(capst) scq(%d) cstr: %d cclr: %d tag: %x tagged: %d addr: %x dir: %d state: %d mask: %x\n",
							...
							cap_exe_req(w).bits.tagged, cap_exe_req(w).bits.addr, cap_exe_req(w).bits.dir,
							s_update, scq(scq_idx).bits.tag_dep_mask)
  ...
  require (xLen >= fLen) // for correct SDQ size
	...
	for (w <- 0 until memWidth) {
		temp_ldst_traffic = Mux(fired_dmem_req_ldst(w),
														temp_ldst_traffic + 1.U, temp_ldst_traffic)
		temp_bounds_traffic = Mux(fired_dmem_req_bounds(w),
  ...
  r_xcpt.badvaddr    := Mux(use_mem_xcpt || use_ld_xcpt, mem_xcpt_vaddr, scq_fire_e.bits.addr) //TODO check addr

	when (cstr_xcpt_valid) {
		printf("[%d] Found cstr_xcpt_valid! scq(%d) rob(%d)\n",
							io.core.tsc_reg, scq_head, scq_fire_e.bits.uop.rob_idx)
	}
  ...
  for (w <- 0 until memWidth) {
    ...
    when (io.dmem.cap_nack(w).valid) {
      ...
      when (cap_nack_uop.cap_cmd === CAP_LD) {
        ...
        scq(scq_idx).bits.state := Mux(scq(scq_idx).bits.state === s_wait, s_fire, s_fire2)
				...
				//when (IsOlder(cap_nack_uop.scq_idx, scq_execute_head, scq_head)) {
				//	scq_execute_head := cap_nack_uop.scq_idx
				//}

        printf("[%d] Received Nack! scq(%d)\n",
    ...
    when (io.dmem.cap_resp(w).valid) {
      ...
      val scq_idx = io.dmem.cap_resp(w).bits.uop.scq_idx
			...
			val resp_lbnd = io.dmem.resp(w).bits.dataBeats(vaddrBits-1,0)
			val resp_ubnd = (resp_lbnd + io.dmem.resp(w).bits.dataBeats(xLen+vaddrBits-1,xLen))
      ...
      when (io.dmem.cap_resp(w).bits.uop.cap_cmd === CAP_LD) {
				slq(slq_idx).bits.state := Mux(decr, s_fire, s_done)
				slq(slq_idx).bits.failed := fail
				// TODO just use WrapDecWay?
				slq(slq_idx).bits.way := Mux(decr, WrapIncOrDecWay(slq(slq_idx).bits.dir, slq(slq_idx).bits.way, num_ways),
																			slq(slq_idx).bits.way)
				slq(slq_idx).bits.count := Mux(decr, slq(slq_idx).bits.count - 1.U, slq(slq_idx).bits.count)
				
				printf("slq(%d) addr: %x resp_lbnd: %x resp_ubnd: %x\n", slq_idx, addr, resp_lbnd, resp_ubnd)
				assert(slq(slq_idx).bits.state === s_wait)
      } .elsewhen (io.dmem.cap_resp(w).bits.uop.cap_cmd === CAP_ST) {
				ssq(ssq_idx).bits.state := Mux(decr, s_fire, s_done)
				...
				printf("ssq(%d) addr: %x resp_lbnd: %x resp_ubnd: %x\n", ssq_idx, addr, resp_lbnd, resp_ubnd)
				assert(ssq(ssq_idx).bits.state === s_wait)
			} .otherwise {
				scq(scq_idx).bits.state := Mux(occ_chk, Mux(scq(scq_idx).bits.state === s_wait, s_fire2, s_done),
																		Mux(decr, s_fire, s_done /* fail */))
				scq(scq_idx).bits.failed := fail
				scq(scq_idx).bits.way := Mux(decr, WrapIncOrDecWay(scq(scq_idx).bits.dir, scq(scq_idx).bits.way, num_ways),
																			scq(scq_idx).bits.way)
				scq(scq_idx).bits.count := Mux(decr, scq(scq_idx).bits.count - 1.U, scq(scq_idx).bits.count)

        scq(scq_idx).bits.succeeded := (occ_chk && scq(scq_idx).bits.state === s_wait2)
				printf("scq(%d) addr: %x resp_data: %x next_state: %d\n",
								scq_idx, addr, resp_data, Mux(occ_chk, Mux(scq(scq_idx).bits.state === s_wait, s_fire2, s_done),
								Mux(decr, s_fire, s_done /* fail */)))
				assert(scq(scq_idx).bits.state === s_wait || scq(scq_idx).bits.state === s_wait2)
			}

			// Update ccache
			ccache_write_val(w) := (cap_chk | (occ_chk & is_cclr))
			ccache_write_tag(w) := Mux(is_load, slq(slq_idx).bits.tag,
															Mux(is_store, ssq(ssq_idx).bits.tag, scq(scq_idx).bits.tag))
			ccache_write_data(w) := Mux(is_cclr, 0.U, Cat(resp_ubnd, resp_lbnd))
    ...
    when (scq(i).valid)
    {
      ...
      when (IsKilledByBranch(io.core.brupdate, scq(i).bits.uop))
      {
        ...
        sc_brkilled_mask(i)    := true.B
				...
				scq(i).bits.tagged     := false.B
				scq(i).bits.failed     := false.B
				scq(i).bits.succeeded  := false.B
  ...
  for (w <- 0 until coreWidth)
  {
    ...
    temp_ldq_head        = Mux(commit_load,
		...
		//yh+begin
		temp_num_load = Mux(enableStat & commit_load, 
													temp_num_load + 1.U, temp_num_load)
		//yh+end
  }
  ...
  var temp_scq_commit_head = scq_commit_head
  ...
  for (w <- 0 until coreWidth) {
    ...
    when (commit_ldchk) {
      ...
      printf("[%d] Commit slq(%d)\n", io.core.tsc_reg, idx)
			assert(io.core.commit.uops(w).cap_cmd === CAP_LD)
    } .elsewhen (commit_stchk) {
	    val idx = temp_ssq_commit_head
      ssq(idx).bits.committed := true.B

      printf("[%d] Commit ssq(%d)\n", io.core.tsc_reg, idx)
			assert(io.core.commit.uops(w).cap_cmd === CAP_ST)
    } .elsewhen (commit_capst) {
	    val idx = temp_scq_commit_head
      scq(idx).bits.committed := true.B

      printf("[%d] Commit scq(%d)\n", io.core.tsc_reg, idx)
			assert(io.core.commit.uops(w).cap_cmd === CAP_CS ||
							io.core.commit.uops(w).cap_cmd === CAP_CC)
		}

    temp_slq_commit_head = Mux(commit_ldchk,
                               WrapInc(temp_slq_commit_head, numSlqEntries),
                               temp_slq_commit_head)

    temp_ssq_commit_head = Mux(commit_stchk,
                               WrapInc(temp_ssq_commit_head, numSsqEntries),
                               temp_ssq_commit_head)

    temp_scq_commit_head = Mux(commit_capst,
                               WrapInc(temp_scq_commit_head, numScqEntries),
                               temp_scq_commit_head)
  }

  slq_commit_head := temp_slq_commit_head
  ssq_commit_head := temp_ssq_commit_head
  scq_commit_head := temp_scq_commit_head
  ...
  when (clear_store)
  {
	...
	//yh+begin
	num_store := Mux(initStat, io.core.dpt_csrs.num_store,
									Mux(enableStat & clear_store, num_store + 1.U, num_store))

  clear_ldchk := (slq(slq_head).valid && slq(slq_head).bits.state === s_done &&
  ...
  when (clear_ldchk) {
	...
	clear_load_check_r1 := clear_ldchk
	clear_slq_tagged_r1 := slq(slq_head).bits.tagged
	clear_slq_ccache_hit_r1 := slq(slq_head).bits.ccache_hit

	num_tagged_load := Mux(initDPT, io.core.dpt_csrs.num_tagged_load,
                        Mux(clear_load_check_r1 && clear_slq_tagged_r1, 
																num_tagged_load + 1.U, num_tagged_load))
  num_load_hit := Mux(initDPT, io.core.dpt_csrs.num_load_hit,
                        Mux(clear_load_check_r1 && clear_slq_tagged_r1 && clear_slq_ccache_hit_r1,
																num_load_hit + 1.U, num_load_hit))

  clear_stchk := (ssq(ssq_head).valid && ssq(ssq_head).bits.state === s_done &&
                        ssq(ssq_head).bits.committed)
  ...
  when (clear_stchk) {
	...
	val clear_stchk_r1 = Reg(Bool())
	val clear_ssq_tagged_r1 = Reg(Bool())
	val clear_ssq_ccache_hit_r1 = Reg(Bool())
	clear_stchk_r1 := clear_stchk
	clear_ssq_tagged_r1 := ssq(ssq_head).bits.tagged
	clear_ssq_ccache_hit_r1 := ssq(ssq_head).bits.ccache_hit

	num_tagged_store := Mux(initStat, io.core.dpt_csrs.num_tagged_store,
                        Mux(enableStat & clear_stchk_r1 & clear_ssq_tagged_r1,
														num_tagged_store + 1.U, num_tagged_store))

	num_store_hit := Mux(initStat, io.core.dpt_csrs.num_store_hit,
                      Mux(enableStat & clear_stchk_r1 & clear_ssq_tagged_r1 & clear_ssq_ccache_hit_r1,
											  num_store_hit + 1.U, num_store_hit))

  clear_capst := (scq(scq_head).valid && scq(scq_head).bits.state === s_done &&
                        scq(scq_head).bits.committed && scq(scq_head).bits.succeeded)

  when (clear_capst) {
    scq(scq_head).valid           := false.B
    scq(scq_head).bits.state			:= s_init
    scq(scq_head).bits.tagged     := false.B
    scq(scq_head).bits.failed     := false.B
		scq(scq_head).bits.succeeded	:= false.B
		scq(scq_head).bits.committed	:= false.B

    scq_head := WrapInc(scq_head, numScqEntries)
    printf("[%d] Clear head scq(%d)\n", io.core.tsc_reg, scq_head)
  }

  //num_cstr := Mux(initDPT, io.core.dpt_csrs.num_cstr,
	//								Mux(enableStat & clear_capst & (scq(scq_head).bits.uop.cap_cmd === CAP_CS),
	//										num_cstr + 1.U, num_cstr))

  //num_cclr := Mux(initDPT, io.core.dpt_csrs.num_cclr,
	//								Mux(enableStat & clear_capst & (scq(scq_head).bits.uop.cap_cmd === CAP_CC),
	//										num_cclr + 1.U, num_cclr))

  //num_cstr_itr := Mux(initDPT, io.core.dpt_csrs.num_cstr_itr,
	//								Mux(enableStat & clear_capst & (scq(scq_head).bits.uop.cap_cmd === CAP_CS),
	//										num_cstr_itr + (init_count - scq(scq_head).bits.count + 1.U), num_cstr_itr))

  //num_cclr_itr := Mux(initDPT, io.core.dpt_csrs.num_cclr_itr,
	//								Mux(enableStat & clear_capst & (scq(scq_head).bits.uop.cap_cmd === CAP_CC),
	//										num_cclr_itr + (init_count - scq(scq_head).bits.count + 1.U), num_cclr))

  when (clear_capst) {
    val tidx = (scq(scq_head).bits.tag(tagWidth/2-1,0))

		when (scq(scq_head).bits.uop.cap_cmd === CAP_CS) {
			val way = WrapIncWay(scq(scq_head).bits.way, num_ways)
			shb_meta(tidx) := scq(scq_head).bits.tag(tagWidth-1,tagWidth/2)
			shb_data(tidx) := way
			printf("[%d] Update SHB[%x]: %d\n", io.core.tsc_reg, tidx, way)
		}

		when (scq(scq_head).bits.uop.cap_cmd === CAP_CC) {
			val way = WrapIncOrDecWay(scq(scq_head).bits.dir, scq(scq_head).bits.way, num_ways)
			chb_meta(tidx) := scq(scq_head).bits.tag(tagWidth-1,tagWidth/2)
			chb_data(tidx) := way
	...
	when (enableDPT) {
		printf("[%d] slq_head: %d slq_tail: %d ssq_head: %d ssq_tail: %d scq_head: %d scq_tail: %d scq_execute_head: %d\n",
						io.core.tsc_reg, slq_head, slq_tail, ssq_head, ssq_tail, scq_head, scq_tail, scq_execute_head)
  ...
  //yh+begin
  val sc_exc_killed_mask = WireInit(VecInit((0 until numSsqEntries).map(x=>false.B)))
  ...
  when (reset.asBool || io.core.exception) {
    when (reset.asBool) {
      ...
      scq_tail := 0.U
      scq_commit_head  := 0.U
      scq_execute_head := 0.U

      for (i <- 0 until numScqEntries) {
        scq(i).valid           := false.B
        scq(i).bits.data.valid := false.B
        scq(i).bits.uop        := NullMicroOp
				scq(i).bits.state			 := s_init
				scq(i).bits.tagged     := false.B
				scq(i).bits.failed     := false.B
				scq(i).bits.succeeded  := false.B
				scq(i).bits.committed  := false.B
      ...
      for (i <- 0 until numSsqEntries) {
        when (!ssq(i).bits.committed) {
          ...
          ssq(i).bits.state := s_update
        }
      }

      scq_tail := scq_commit_head

      for (i <- 0 until numScqEntries) {
        when (!scq(i).bits.committed) {
          scq(i).valid           := false.B
          scq(i).bits.data.valid := false.B
          sc_exc_killed_mask(i)  := true.B
					scq(i).bits.state			 := s_init
					scq(i).bits.tagged     := false.B
					scq(i).bits.failed     := false.B
					scq(i).bits.succeeded	 := false.B

					printf("[%d] Exception inits scq(%d)\n", io.core.tsc_reg, i.U)
        }
      }
    }
  }
  ...
  live_tag_mask := (next_live_tag_mask &
                    ~(sc_brkilled_mask.asUInt) &
                    ~(sc_exc_killed_mask.asUInt))
