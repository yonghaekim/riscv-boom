  ...
  for (w <- 0 until coreWidth) {
    ...
    when (io.enq_valids(w)) {
      ...
      rob_bsy(rob_tail)       := !(io.enq_uops(w).is_fence 	||
                                   ...
                                   io.enq_uops(w).cap_cmd(1) /* cstr, cclr */)
      //yh-rob_unsafe(rob_tail)    := io.enq_uops(w).unsafe
      rob_unsafe(rob_tail)    := (io.enq_uops(w).unsafe && !io.enq_uops(w).cap_cmd(1))
			when (io.enq_uops(w).is_fence	||
							io.enq_uops(w).is_fencei || 
              io.enq_uops(w).cap_cmd === CAP_CS ||
              io.enq_uops(w).cap_cmd === CAP_CC) {
				printf("rob_bsy(%d) is false cstr: %d cclr: %d\n",
                rob_tail, io.enq_uops(w).cap_cmd === CAP_CS, io.enq_uops(w).cap_cmd === CAP_CC)
			}
			//yh+end
...
//                  "FP instruction writing back exc bits is overriding an existing exception.")
    ...
    when (io.lxcpt.valid && MatchBank(GetBankIdx(io.lxcpt.bits.uop.rob_idx))) {
      ...
      when (io.lxcpt.bits.cause =/= MINI_EXCEPTION_MEM_ORDERING && io.lxcpt.bits.uop.cap_cmd =/= CAP_CS) {
