        ...
        BoomTileAttachParams(
          tileParams = BoomTileParams(
            core = BoomCoreParams(
              ...
              //yh+end
              //yh-maxBrCount = 8,
              //yh-numFetchBufferEntries = 8,
              //yh-ftq = FtqParameters(nEntries=16),
              //yh+begin
              maxBrCount = 16,
            ...
            dcache = Some(
              //yh-DCacheParams(rowBits = 64, nSets=64, nWays=4, nMSHRs=2, nTLBWays=8)
              DCacheParams(rowBits = site(SystemBusKey).beatBits, nSets=64, nWays=4, nMSHRs=2, nTLBWays=8) //yh+
            ),
            icache = Some(
              //yh-ICacheParams(rowBits = 64, nSets=64, nWays=4, fetchBytes=2*4)
              ICacheParams(rowBits = site(SystemBusKey).beatBits, nSets=64, nWays=4, fetchBytes=4*4) //yh+
            ),
            hartId = i + idOffset
          ),
